...
namespace Compiler
{
    public partial class MainForm : Form
    {
        ...
        private void Run()
        {
            ...
            string[] keywords = { "final" };
            string[] keywords_double = { "double" };
            string[] delimiters = { " " };
            string[] point = { "." };
            string[] equallies = { "=" };
            string[] pluses = { "+" };
            ...
            while (position < input.Length)
            {
                ...
                //val
                foreach (string keyword_val in keywords_double)
                {
                ...
                foreach (string pointL in point)
                {
                    if (input.Substring(position).StartsWith(pointL))
                    {
                        lexemes.Add(new Lexeme(lexemeCodes[LexemeType.Point], LexemeType.Point, input, position, position + pointL.Length - 1));
                        position += pointL.Length;
                        found = true;
                        break;
                    }
                }
