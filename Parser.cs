    ...
    public class Parser
    {
        ...
        private void DEF(DataGridView dataGridView1)
        {
            try
            {
                ...
                if (res == 0)
                {
                    dataGridView1.Rows.Add("Ожидалось ключевое слово 'final'", lexemes[position].StartPosition);
                    counter++;
        ...
        private void VAL(DataGridView dataGridView1)
        {
            try
            {
                ...
                if (res == 0)
                {
                    dataGridView1.Rows.Add($"Ожидалось ключевое слово double", lexemes[position].StartPosition);
                    counter++;
        ...
        private void POINT(DataGridView dataGridView1)
        {
            try
            {
                int res = 0;
                for (int u = position; u < flag; u++)
                {
                    if (lexemes[u].Type == LexemeType.Point)
                    {
                        res = 1;
                        break;
                    }
                ...
                if (res == 0)
                {
                    //dataGridView1.Rows.Add($"Ожидалась точка", lexemes[position - 1].EndPosition + 1);
                    //counter++;
                ...
                else
                {
                    ...
                    if (lexemes[position].Type == LexemeType.Point)
                    {
                        str += lexemes[position].Token;
                        position++;
                        POINTREM(dataGridView1);
                    }
                    else if (lexemes[position].Type == LexemeType.Invalid)
                    {
                        dataGridView1.Rows.Add($"Недопустимый символ '{lexemes[position].Token}'", lexemes[position].StartPosition);
                    ...
                    else if (lexemes[position].Type != LexemeType.Point)
                    {
                        dataGridView1.Rows.Add($"Cимвол '{lexemes[position].Token}'", lexemes[position].StartPosition);
                        counter++;
                        position++;
        ...
        private void POINTREM(DataGridView dataGridView1)
        {
            try
            {
